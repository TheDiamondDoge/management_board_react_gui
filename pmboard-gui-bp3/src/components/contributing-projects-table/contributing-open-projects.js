...
export default class ContributingOpenProjects extends React.Component {
    render() {
        const {offerName, milestones} = this.props.offer;
        ...
        const tds = this.getMilestonesPerMonth(monthsBetween, min, milestones);
    ...
    getMilestonesPerMonth(monthsBetween, min, milestones) {
        ...
        for (let i = 0; i < monthsBetween; i++) {
            ...
            tds[i] = {milestones: filteredMils, needYearBorder}
        }
        return tds;
    }
    ...
    getMilestonesPerMonthForProducts(monthsBetween, min) {
        ...
        for (let i = 0; i < contributed.length; i++) {
            products[contributed[i].projectName] = this.getMilestonesPerMonth(monthsBetween, min, contributed[i].milestones);
        }
